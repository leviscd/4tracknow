...
const Header: React.FC<Props> = ({ user, remaining, onTabChange, activeTab }) => {
    ...
    <header className="w-full border-b border-purple-500/10 bg-white/70 dark:bg-[#050508]/70 backdrop-blur-md sticky top-0 z-50 transition-all duration-300">
      ...
      <span className="font-orbitron text-2xl font-bold tracking-wider text-foreground">
        ...
        <nav className="hidden md:flex items-center gap-1 p-1 rounded-full border border-purple-500/10 bg-white/50 dark:bg-white/5 backdrop-blur-sm">
          <button
            ...
            className={`px-5 py-2 text-xs font-semibold uppercase tracking-widest rounded-full transition-all ${
              activeTab === 'painel'
                ? 'bg-purple-500/10 text-purple-600 dark:text-purple-300 dark:bg-purple-500/20 border border-purple-500/20 shadow-glow-sm'
                : 'text-slate-500 dark:text-slate-400 hover:text-purple-500 dark:hover:text-purple-300 hover:bg-white/40 dark:hover:bg-white/5'
            }`}
          >
            Painel
          </button>
        ...
        <div className="flex items-center gap-4">
          ...
          {isAdmin && (
            <span className="hidden lg:inline-block text-xs font-bold uppercase tracking-wider text-purple-600 dark:text-purple-400 bg-purple-100 dark:bg-purple-500/20 border border-purple-200 dark:border-purple-500/30 rounded px-2 py-1">
              ♔ DONO
            </span>
          )}
      ...
      <nav
        id="mobile-menu"
        className={`md:hidden flex-col gap-2 p-4 bg-white/70 dark:bg-[#050508]/70 backdrop-blur-md border-t border-purple-500/10 transition-all duration-300 ${
        ...
        {isAdmin && (
          ...
          >
            <span className="flex items-center gap-2">
             ♔ DONO
              <span className="text-xs text-purple-500">(restrito)</span>
            </span>
          </button>
        )}
      </nav>
